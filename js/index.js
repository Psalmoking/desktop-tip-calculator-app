...
const tipCalculator = function () {
    ...
    const updateSummaryUI = function (tip, total) {
        tipPerPersonEl.textContent = `${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(tip)}`;
        totalPerPersonEl.textContent = `${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(total)}`;
    };
